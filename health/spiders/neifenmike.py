...
#            ]
    ...
    def start_requests(self):
        pages=[]
        for npage in range(0, 55):
            if npage == 0:
                page = scrapy.Request("http://jbk.39.net/bw/neifenmike")
            else:
                #npage = npage + 1
                un = str(npage)
                header = "/bw/neifenmike_p"
                tail = "#ps"
    ...
    def parse(self, response):
        #jbItem = HealthItem()
        print response.url
        for hel in response.xpath('//div[@id="res_tab_1"]/div[@class="res_list"]'):
